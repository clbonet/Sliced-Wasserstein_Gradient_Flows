...
def one_step_flowCNN(n_epochs, rho_prev, J, d, h, 
                  num_projections, d_latent, lr, n_samples, device, 
    ...
    if k>0 and not reset_nn:
        ...
        rho_k = CNN(noise_dim=d_latent).to(device)
    ...
    for j in range(n_epochs):
        z_k = torch.randn(n_samples, d_latent, 1, 1, device=device)
        x_k = rho_k(z_k)

        if k>0:
            z0 = torch.randn(n_samples, d_latent, 1, 1, device=device)
            x_prev = rho_prev(z0)
        else:
        ...
        if sw_approx:
            ...
            sw = sliced_wasserstein(x_k.reshape(-1,28*28), x_prev.reshape(-1,28*28), num_projections, device, 
...
def SWGFCNN(rho_0, tau, n_step, n_epochs, J, d=2, d_latent=100, lrs=1e-5, 
    ...
    for k in pbar:
        ...
        rho_k = one_step_flowCNN(n_epoch, Lrho[-1], J, d, tau, 
                                num_projections, d_latent, lr, n_samples, 
