...
parser.add_argument("--tau", type=float, default=0.1, help="step size")
...
if __name__=="__main__":
    ...
    symKLs_icnn = np.zeros((len(ts), args.ntry))
    Fs = np.zeros((len(ts), args.ntry))
...
#             print(len(Lrho))
        ...
        for i, t in enumerate(ts):
            ...
            if args.stationary:
                symKLs_icnn[i, k] = np.log10(symKL(Lrho, true_distr, 10000, device))
            else:
                try:
                    symKLs_icnn[i, k] = np.log10(symKL(Lrho[:ind_t+1], true_distr, 10000, device))
                except:
                    symKLs_icnn[i, k] = np.inf
            ...
            Fs[i, k] = torch.abs(f-f_true).detach().cpu().numpy()


        print("d",d,"k",k,flush=True)
        print("SW_ICNN", symKLs_icnn[:, k],flush=True)
        print("Fs", Fs[:, k], flush=True)
        print()

    np.savetxt("./results/gaussians_d_"+str(d), symKLs_icnn, delimiter=",")
    np.savetxt("./results/FS_ICNN_d"+str(d), Fs, delimiter=",")
        
#     m = np.mean(symKLs_icnn, axis=1)
#     s = np.std(symKLs_icnn, axis=1)
#     plt.plot(ts, m)
#     plt.fill_between(ts, m-2*s/np.sqrt(args.ntry), m+2*s/np.sqrt(args.ntry), alpha=0.5)
#     plt.grid(True)
#     plt.xlabel("t")
#     plt.ylabel("$\log_{10}\mathrm{SymKL}$")
#     plt.title("./results/gaussians_symKL_d"+str(d)+".png", format="png")
#     plt.close("all")
...
#     plt.xlabel("t")
#     plt.ylabel("Error")
#     plt.title("./results/Fs_gaussians_d"+str(d)+".png", format="png")
#     plt.close("all")     
